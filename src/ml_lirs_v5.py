    ...
    def LIRS(self, v_time, ref_block):
        ...
        if (self.page_table[ref_block].refer_times > 0):
            ...
            if (self.mini_batch_X.all()):
                self.mini_batch_X = np.array([self.page_table[ref_block].position + [self.page_table[ref_block].last_is_hir]])
                self.mini_batch_Y = np.array([1 if self.page_table[ref_block].recency else -1])
                if (self.page_table[ref_block].recency):
                    self.positive_sampler += 1
                else:
                    self.negative_sampler += 1
            else:
                ...
                self.mini_batch_X = np.r_[self.mini_batch_X, [self.page_table[ref_block].position + [self.page_table[ref_block].last_is_hir]]]
            ...
            if (self.page_table[ref_block].recency0):
                ...
                position[1] = 1
            elif (not self.page_table[ref_block].recency):
                position[2] = 1
            self.page_table[ref_block].position = position
            self.page_table[ref_block].is_last_hit = hit
