...
class WriteToFile:
    def __init__(self, tName, st):
        ...
        try:  
            ...
            print(error)
        self.FILE = open("../result_set/" + self.tName + "/ml_lirs_v9_" + str(st) + "_" + self.tName, "w")
...
class Segment_Miss:
    def __init__(self, tName, cache):
        self.tName = tName
        self.FILE = open("../result_set/" + self.tName + "/ml_lirs_v9_" + self.tName + "_" + str(cache) + "_segment_miss", "w")
...
class Breakdown:
    def __init__(self, tName, cache):
        self.tName = tName
        self.FILE = open("../result_set/" + self.tName + "/ml_lirs_v9_" + self.tName + "_" + str(cache) + "_breakdown", "w")
...
class LIRS_Replace_Algorithm:
    def __init__(self, t_name, vm_size, trace_dict, mem_size, trace_size, model, start_use_model=3000, mini_batch=1000):
        ...
        self.breakdown = Breakdown(t_name, mem_size)
    ...
    def collect_sampler(self, v_time, ref_block, label, *feature):
        self.count_exampler += 1

        features = []
        for f in feature:
            features += f
        # print(v_time, features[0])
        """
    ...
    def LIRS(self, v_time, ref_block):
        ...
        if (v_time > 0 and v_time % self.epoch == 0):
            ...
            # print(self.lir_size)
            # self.check_Rmax0()
            # print(v_time, str(self.out_nega), str(self.out_pos), str(self.in_nega), str(self.in_pos))
            self.breakdown.record(str(self.out_nega), str(self.out_pos), str(self.in_nega), str(self.in_pos))
            self.out_nega = 0
            self.out_pos = 0
            self.in_nega = 0
            self.in_pos = 0
        ...
        if (self.page_table[ref_block].refer_times > 1):
            if (self.page_table[ref_block].recency0):
                ...
                self.collect_sampler(v_time, ref_block, 1, self.page_table[ref_block].position)
            else:
                self.collect_sampler(v_time, ref_block, -1, self.page_table[ref_block].position)
        # print(v_time)
        # when use the data to predict the model
        if (not self.train and self.count_exampler > self.start_use_model and self.Free_Memory_Size == 0 and self.count_exampler > self.mini_batch):
            self.train = True
            print ("Start predict : ", v_time)
