"""
model: naive bayes sklearn.naive_bayes.GaussianNB
...
feature:
1. flattened position 2. is hir before 3. hit/miss
...
class WriteToFile:
    def __init__(self, tName, st):
        ...
        try:  
            ...
            print(error)
        self.FILE = open("../result_set/" + self.tName + "/ml_lirs_v8_" + str(st) + "_" + self.tName, "w")
...
class Segment_Miss:
    def __init__(self, tName, cache):
        self.tName = tName
        self.FILE = open("../result_set/" + self.tName + "/ml_lirs_v8_" + self.tName + "_" +str(cache) + "_segment_miss", "w")
...
class LIRS_Replace_Algorithm:
    ...
    def find_new_Rmax(self):
        ...
        while (self.Rmax.is_hir == True):
            ...
            self.collect_sampler(self.Rmax.block_number, -1, self.Rmax.position, [self.Rmax.last_is_hir], [self.page_table[self.Rmax.block_number].is_last_hit], [self.page_table[self.Rmax.block_number].refer_times])
    ...
    def LIRS(self, v_time, ref_block):
        ...
        if (self.page_table[ref_block].refer_times > 0):
            ...
            if (self.page_table[ref_block].recency):
                ...
                self.collect_sampler(ref_block, 1, self.page_table[ref_block].position, [self.page_table[ref_block].last_is_hir], [self.page_table[ref_block].is_last_hit], [self.page_table[ref_block].refer_times])
            else:
                self.collect_sampler(ref_block, -1, self.page_table[ref_block].position, [self.page_table[ref_block].last_is_hir], [self.page_table[ref_block].is_last_hit], [self.page_table[ref_block].refer_times])
        ...
        # start predict
        if (self.train):
            feature = np.array([self.page_table[ref_block].position + [self.page_table[ref_block].is_hir] + [self.page_table[ref_block].is_last_hit] + [self.page_table[ref_block].refer_times + 1]])
